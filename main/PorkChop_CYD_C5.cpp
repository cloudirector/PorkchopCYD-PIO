...
void init() {
  ...
  if (mainSprite.createSprite(DISPLAY_W, 148)) {
    Serial.println("[DISPLAY] Sprite 320x148 16bpp OK");
  } else if (mainSprite.createSprite(DISPLAY_W, 128)) {
    Serial.println("[DISPLAY] Sprite 320x128 16bpp OK");
  } else {
...
#define TOUCH_HOLD_MS   600
#define TOUCH_DEBOUNCE   20
...
// Touch calibration for NM-CYD-C5
#define T_XMIN  100
#define T_XMAX 3950
#define T_YMIN  100
#define T_YMAX 3950
...
bool getPoint(int& sx, int& sy) {
  ...
  pinMode(TOUCH_CLK_PIN,  OUTPUT); digitalWrite(TOUCH_CLK_PIN,  LOW);
  pinMode(TOUCH_MOSI_PIN, OUTPUT); digitalWrite(TOUCH_MOSI_PIN, LOW);

  uint16_t z1[3];
  for (int i=0; i<3; i++) { z1[i]=_xpt_cmd_raw(0xB0); _xpt_cmd_raw(0xC0); }

  PIN_FUNC_SELECT(IO_MUX_GPIO6_REG, 2);
  PIN_FUNC_SELECT(IO_MUX_GPIO7_REG, 2);

  uint16_t mn=min(z1[0],min(z1[1],z1[2])), mx=max(z1[0],max(z1[1],z1[2]));
  if (mx-mn>300 || mn<900) return false;

  pinMode(TOUCH_CLK_PIN,  OUTPUT); digitalWrite(TOUCH_CLK_PIN,  LOW);
  pinMode(TOUCH_MOSI_PIN, OUTPUT); digitalWrite(TOUCH_MOSI_PIN, LOW);

  uint32_t adcX=0, adcY=0;
  for (int i=0; i<4; i++) { adcX+=_xpt_cmd_raw(0xD0); adcY+=_xpt_cmd_raw(0x90); }

  PIN_FUNC_SELECT(IO_MUX_GPIO6_REG, 2);
  PIN_FUNC_SELECT(IO_MUX_GPIO7_REG, 2);

  uint16_t rX=adcX/4, rY=adcY/4;
  if (rX>4000 && rY>4000) return false;

  sx = map(rX, T_XMIN, T_XMAX, 0, DISPLAY_W);
  sy = map(rY, T_XMAX, T_XMIN, 0, DISPLAY_H);
...
// ============================================================
void handleInput() {
  static uint8_t _idleCount = 0;
  static bool _inputReady = false;
  if (!_inputReady) {
    int _dx, _dy;
    if (!Touch::getPoint(_dx, _dy)) _idleCount++; else _idleCount = 0;
    if (_idleCount >= 30) _inputReady = true;
    return;
  }
  ...
  switch (currentMode) {
    ...
    case PorkchopMode::MENU: {
      ...
      if (isTap) {
        ...
        int sprH = mainSprite.created() ? mainSprite.height() : MAIN_H;
        int zone = (relY < sprH/3) ? 0 : (relY > sprH*2/3) ? 2 : 1;
        if (zone == 0) {
          // Scroll up
    ...
    case PorkchopMode::ACHIEVEMENTS: {
      if (isTap) {
        if (achDetailShow) {
          ...
          if (achScroll + 7 < ACH_COUNT) { achScroll++; achDetailIdx = achScroll; }
        } else {
...
void setup() {
  ...
  Serial.begin(115200);
  delay(100);
  Serial.println("\n=== PORKCHOP CYD v6 STARTING === [C5-v27]");
  ...
  delay(20);
  // SD: single attempt, graceful fail — no pin scanning
  sdSPI.begin(6, 2, 7, -1);
  Serial.printf("[SD]   attempt @ 400kHz...\n");
  if (SD.begin(SD_CS_PIN, sdSPI, 400000)) {
    sdAvailable = true;
    uint8_t ct = SD.cardType();
    Serial.printf("[SD] OK type=%s %lluMB\n",
      ct==CARD_MMC?"MMC":ct==CARD_SD?"SD":ct==CARD_SDHC?"SDHC":"UNKNOWN",
      SD.totalBytes()/(1024*1024));
    if (!SD.exists("/captures")) SD.mkdir("/captures");
    if (!SD.exists("/wigle"))    SD.mkdir("/wigle");
    if (!SD.exists("/logs"))     SD.mkdir("/logs");
  } else {
    SD.end(); sdSPI.end();
    Serial.println("[SD] Not found — skipping");
  }
